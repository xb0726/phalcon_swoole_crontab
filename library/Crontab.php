...
class Crontab
{
    ...
    static public function stop($output = true)
    {
        ...
        if ($pid) {
            if (\swoole_process::kill($pid, 0)) {
                ...
                Output::stderr("进程" . $pid . "已结束");
            } else {
                @unlink(self::$pid_file);
                Output::stderr("进程" . $pid . "不存在,删除pid文件");
            }
        } else {
            $output && Output::stderr("需要停止的进程未启动");
        }
    }
    ...
    static public function load_config()
    {
        ...
        foreach ($config as $id => $task) {
            ...
            if ($ret === false) {
                Output::stdout(ParseCrontab::$error);
            } elseif (!empty($ret)) {
                TickTable::set_task($ret, array_merge($task, ["id" => $id]));
            }
        }
    }
            ...
            while ($ret = \swoole_process::wait(false)) {
                ...
                if (isset(self::$task_list[$pid])) {
                    ...
                    if ($task["type"] == "crontab") {
                        ...
                        $start = $task["start"];
                        $id = $task["id"];
                        Output::stdout("{$id} [Runtime:" . sprintf("%0.6f", $end - $start) . "]");
                    ...
                    if ($task["type"] == "worker") {
                        ...
                        Output::stdout("{$classname}_{$task["number"]} [Runtime:" . sprintf("%0.6f",
                                $end - $start) . "]");
                        $task["process"]->close();//关闭进程
